...
| **E7** | ⬜ Todo | Architettura plug-in + adapter Odoo | Interfaccia adapter (`getProjects`/`getTasks` per autocomplete live), registry estendibile, integrazione Odoo (JSON-RPC), gestione errori parziali. **Seed di una config Odoo di default** per sbloccare l'MVP employee senza pannello Admin. | `backend_configs` | Tech |
