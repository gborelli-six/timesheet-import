...
- `UniqueConstraint(user_id, service)` → `uq_user_tokens_user_id`: al massimo un connettore per servizio per utente.
...
- **AAD (Associated Additional Data)** = `f"{user_id}:{service}".encode()`, passato come `associated_data` sia in cifratura sia in decifratura. Lega il ciphertext al record specifico: un blob `secret_enc` copiato su un altro `user_id`/`service` fa fallire la decifratura con `InvalidTag`. Difesa contro tampering e "ciphertext swap" a livello DB. **`user_id` per l'AAD deriva sempre dal JWT di sessione, mai dal body/query** (così l'AAD non è influenzabile dal client).

Helper in `backend/app/core/security.py` (oggi stub):

```python
def encrypt_secret(plaintext: str, user_id: int, service: str) -> tuple[bytes, bytes, int]:
    """Ritorna (secret_enc, nonce, key_version)."""

def decrypt_secret(secret_enc: bytes, nonce: bytes, user_id: int, service: str, key_version: int) -> str:
...
- **`GET /api/me/connectors`** → lista di `{service, account_identifier, configured: bool, updated_at}`. **Il segreto non è mai serializzato**, in nessuna risposta. `configured` indica solo se un segreto è presente.
- **`PUT /api/me/connectors/{service}`** → body `{account_identifier?, secret?}`, semantica upsert:
  - `secret` valorizzato → cifra e sostituisce, generando un **nuovo `nonce`**;
  - `secret` assente / `null` su record **esistente** → il segreto resta **invariato** (consente di aggiornare il solo `account_identifier`);
  - `secret` assente / `null` su record **inesistente** → si crea il connettore con `configured=false` (solo identificativo, nessun segreto ancora); l'import resterà bloccato finché il segreto non viene fornito;
  - `account_identifier` aggiornabile liberamente, in chiaro.
- **`DELETE /api/me/connectors/{service}`** → rimuove il connettore.
...
- Estensione del modello per backend che richiedono più di un segreto (es. OAuth con refresh token) — rinviata a quando un adapter concreto lo imporrà.
